...
installTools() {
	...
	if [[ ! -d "$HOME/.acme.sh" ]] || [[ -d "$HOME/.acme.sh" && -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
		...
		if [[ ! -d "$HOME/.acme.sh" ]] || [[ -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
			...
			echoContent red "  1.获取脚本文件失败，请等待作者恢复后尝试"
			echoContent red "  2.搭建脚本出现bug了 "
			exit 0
		fi
	fi
}
...
initTLSNginxConfig() {
	...
	if [[ -n "${currentHost}" ]]; then
		...
		if [[ "${historyDomainStatus}" == "y" ]]; then
			...
			echo
			echoContent yellow "请输入要配置的域名 例: www.kehu33.asia --->"
			read -r -p "域名:" domain
		fi
	else
		echo
...
checkIP() {
	...
	if [[ -z ${localIP} ]] || ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q '\.' && ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q ':'; then
		...
		echoContent yellow " --->  2.检查域名dns解析是否正确，有没有关闭小云朵，也叫cdn"
...
customCDNIP() {
	...
	echoContent yellow "# 注意事项"
	echoContent yellow "\n教程地址:"
	echoContent skyBlue "www.kehu33.asia"
...
EOF
			...
			echoContent green "    https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=vless%3A%2F%2F${id}%40${currentHost}%3A${currentDefaultPort}%3Fencryption%3Dnone%26security%3Dxtls%26type%3Dtcp%26${currentHost}%3D${currentHost}%26headerType%3Dnone%26sni%3D${currentHost}%26flow%3Dxtls-rprx-direct%23${email}\n"
...
EOF
			...
			echoContent green "    https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=vless%3A%2F%2F${id}%40${currentHost}%3A${currentDefaultPort}%3Fencryption%3Dnone%26security%3Dxtls%26type%3Dtcp%26${currentHost}%3D${currentHost}%26headerType%3Dnone%26sni%3D${currentHost}%26flow%3Dxtls-rprx-splice%23${email/direct/splice}\n"
...
EOF
			...
			echoContent green "    https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=vless%3a%2f%2f${id}%40${currentHost}%3a${currentDefaultPort}%3fsecurity%3dtls%26encryption%3dnone%26host%3d${currentHost}%26headerType%3dnone%26type%3dtcp%23${email}\n"
		fi
...
EOF
		...
		echoContent green "    https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=trojan%3A%2F%2F${id}%40${currentHost}%3A${currentDefaultPort}%3Fencryption%3Dnone%26security%3Dxtls%26type%3Dtcp%26${currentHost}%3D${currentHost}%26headerType%3Dnone%26sni%3D${currentHost}%26flow%3Dxtls-rprx-direct%23${email}\n"
...
EOF
		echoContent green "    https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=vmess://${qrCodeBase64Default}\n"
...
EOF
		...
		echoContent green "    https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=vless%3A%2F%2F${id}%40${currentAdd}%3A${currentDefaultPort}%3Fencryption%3Dnone%26security%3Dtls%26type%3Dws%26host%3D${currentHost}%26sni%3D${currentHost}%26path%3D%252f${currentPath}ws%23${email}"
...
EOF
		...
		echoContent green "    https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=vless%3A%2F%2F${id}%40${currentAdd}%3A${currentDefaultPort}%3Fencryption%3Dnone%26security%3Dtls%26type%3Dgrpc%26host%3D${currentHost}%26serviceName%3D${currentPath}grpc%26path%3D${currentPath}grpc%26sni%3D${currentHost}%26alpn%3Dh2%23${email}"
...
EOF
		...
		echoContent green "    https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=trojan%3a%2f%2f${id}%40${currentHost}%3a${port}%3fpeer%3d${currentHost}%26sni%3d${currentHost}%26alpn%3Dhttp/1.1%23${email}\n"
	...
	if [[ -n "${configPath}" ]]; then
		...
		if [[ -n $(ls /etc/v2ray-agent/subscribe/) ]]; then
			find /etc/v2ray-agent/subscribe/* | while read -r email; do
				...
				echoContent yellow "在线二维码:https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=https://${currentDomain}/s/${email}\n"
