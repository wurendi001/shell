...
checkOS(){
    ...
    if [ -n "$ifCentOS" ];then
        ...
        if [[ "$OS_Version" -lt "7" ]];then
            echo -e "${Font_Red}此脚本不支持CentOS${OS_Version},请升级至CentOS8或9或更换其他操作系统${Font_Suffix}"
...
function ScriptTitle() {
    ...
    echo -e "${Font_Green}BUG反馈或使用交流可加TG群组${Font_Suffix} ${Font_Yellow}https://https://t.me/vpsjiaoliudajian ${Font_Suffix}"
    echo ""
    echo -e " ** 测试时间: $(date)"
    echo ""
}
