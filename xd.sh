...
function install_bot(){
    ...
    echo -e "${Green_font_prefix}请输入牛逼ApiToken"
    echo -e "${Green_font_prefix}-----------------------------------"
...
function update_do(){
    ...
    echo "平台架构为${bit}"
    if [[ ${bit} == "arm64" ]]; then
        docker pull docker.xdmb.cloud/xiandan/release/arm:${vNum}
        docker run --platform linux/${bit} --log-opt max-size=10m --dns=${dns} --log-driver json-file --restart=always --name=xiandan -v /etc/xdzz:/xiandan -p ${port}:8080 -d docker.xdmb.cloud/xiandan/release/arm:${vNum}
    else
        docker pull docker.xdmb.cloud/xiandan/release:${vNum}
        docker run --platform linux/${bit} --log-opt max-size=10m --dns=${dns} --log-driver json-file --restart=always --name=xiandan -v /etc/xdzz:/xiandan -p ${port}:8080 -d docker.xdmb.cloud/xiandan/release:${vNum}
    fi
#     if ! type crontab >/dev/null 2>&1; then 
#         check_sys
#         if [[ ${release} == "centos" ]]; then
...
# 	fi
#     ip=`curl -4 ip.sb`
    echo -e "${Green_font_prefix}牛逼面板已安装成功！请等待1-2分钟后访问面板入口。${Font_color_suffix}"
...
function update_do_bot {
    ...
    echo "平台架构为${bit}"
    if [[ ${bit} == "arm64" ]]; then
        docker pull docker.xdmb.cloud/xiandan/bot/arm:latest
        docker run --platform linux/${bit} --name xd-bot --network host --restart=always -v /etc/xdzz/bot_config:/usr/src/app/bot_config -d docker.xdmb.cloud/xiandan/bot/arm:latest
    else
        docker pull docker.xdmb.cloud/xiandan/bot:latest
        docker run --platform linux/${bit} --name xd-bot --network host --restart=always -v /etc/xdzz/bot_config:/usr/src/app/bot_config -d docker.xdmb.cloud/xiandan/bot:latest
    fi
    exit 0
}
...
function uninstall(){
    rm -rf /etc/xdzz
    docker rm -f xiandan
    echo -e "${Green_font_prefix}牛逼已成功卸载${Font_color_suffix}"
}
...
function nodeuninstall() {
    ...
    rm -rf /etc/xiandan
    echo -e "${Green_font_prefix}牛逼节点已成功卸载，规则已删除，请将服务器从面板上删除！${Font_color_suffix}"
}

function uninstall_bot(){
    rm -rf /etc/xdzz/bot_config
    docker rm -f xd-bot
    echo -e "${Green_font_prefix}牛逼BOT已成功卸载${Font_color_suffix}"
}

function start(){
	portinfo=`docker ps -a |grep xiandan`
	if [[ ! -n "$portinfo" ]];then
		echo -e "${Green_font_prefix}牛逼面板未安装,请安装面板！${Font_color_suffix}"
	else
...
function auto() {
    check_root
    echo && echo -e "${Green_font_prefix}       牛逼面板 一键脚本
   ${Green_font_prefix} ----------- 失落的梦 -----------
   ${Green_font_prefix}1. 安装 牛逼面板
   ${Green_font_prefix}2. 更新 牛逼面板
   ${Green_font_prefix}3. 卸载 牛逼面板
  ————————————
   ${Green_font_prefix}4. 启动 牛逼面板
   ${Green_font_prefix}5. 停止 牛逼面板
   ${Green_font_prefix}6. 重启 牛逼面板
  ————————————
